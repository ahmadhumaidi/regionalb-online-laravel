...
class TargetController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        foreach ($items as $item) {
            ...
            RsmMonthlyTarget::updateOrCreate(['area' => $area, 'target_month' => $data['target_month'], 'scope_key' => $key], ['scope_type' => $scope, 'wilayah' => $item['wilayah'] ?: null, 'unit_name' => $item['unit_name'] ?: null, 'staff_name' => $item['staff_name'] ?: null, 'target_leads' => (int) ($data['target_leads'] ?? 0), 'target_follow_up' => (int) ($data['target_follow_up'] ?? 0), 'target_registrasi' => (int) ($data['target_registrasi'] ?? 0), 'target_herregistrasi' => (int) ($data['target_herregistrasi'] ?? 0), 'target_anggaran' => (float) ($data['target_anggaran'] ?? 0), 'notes' => trim((string) ($data['notes'] ?? '')) ?: null, 'created_by_user_id' => $user->id, 'created_by_name' => $user->name]);
        }
