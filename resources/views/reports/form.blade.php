    ...
    <section class="rounded-2xl glass-card p-5">
        ...
        <form method="POST" enctype="multipart/form-data" action="{{ $editing ? route('reports.update', $report) : route($storeRoute) }}" class="grid gap-4 md:grid-cols-2">
            ...
            @if ($config['label'] === 'anggaran' && $editing)
                ...
                @endif
                @if (in_array('status', $adsFields))
                    @php $statusValue = old('status', $report->status); @endphp
                    @php $statusIsAutomatic = $statusValue && ! in_array($statusValue, \App\Services\Reports\ReportFormService::ADS_STATUS_OPTIONS, true); @endphp
                    <div class="grid gap-1 text-sm">
                        <label class="grid gap-1">Status<select name="status" class="rounded-lg border-border bg-surface-muted">@if ($statusIsAutomatic)<option value="{{ $statusValue }}" selected disabled>{{ $statusValue }} (otomatis)</option>@endif @foreach (\App\Services\Reports\ReportFormService::ADS_STATUS_OPTIONS as $option)<option @selected($statusValue === $option)>{{ $option }}</option>@endforeach</select></label>
                        @if ($statusIsAutomatic)<p class="text-xs text-ink-muted">Status ini otomatis dari sistem (bukan pilihan manual) — pilih status lain di atas untuk mengubahnya.</p>@endif
                    </div>
                @endif
                ...
                @if (in_array('attachment_path', $adsFields))
                    <div class="md:col-span-2 rounded-xl border border-l-4 border-border border-l-tone-blue bg-surface-muted/50 p-4">
                        <label class="grid gap-1 text-sm"><span class="flex items-center gap-1.5 font-semibold text-ink"><x-icon name="document" class="h-4 w-4 text-tone-blue" />Upload bukti invoice/screenshot</span><input type="file" name="attachment_path" accept="image/jpeg,image/png,image/webp,application/pdf" class="rounded-lg border-border bg-surface-muted"><span class="text-xs text-ink-muted">JPG, PNG, WEBP, atau PDF; maksimal 5 MB.</span></label>
                ...
                @if (in_array('insight_attachment_path', $adsFields))
                    <div class="md:col-span-2 rounded-xl border border-l-4 border-border border-l-tone-orange bg-surface-muted/50 p-4">
                        <label class="grid gap-1 text-sm"><span class="flex items-center gap-1.5 font-semibold text-ink"><x-icon name="bolt" class="h-4 w-4 text-tone-orange" />Upload bukti insight/pengeluaran iklan</span><input type="file" name="insight_attachment_path" accept="image/jpeg,image/png,image/webp,application/pdf" class="rounded-lg border-border bg-surface-muted"><span class="text-xs text-ink-muted">Screenshot insight/spend dari platform iklan; JPG, PNG, WEBP, atau PDF, maksimal 5 MB.</span></label>
                ...
                @if (in_array('ad_leads_file', $adsFields))
                    <div class="md:col-span-2 rounded-xl border border-l-4 border-border border-l-tone-green bg-surface-muted/50 p-4">
                        <label class="grid gap-1 text-sm"><span class="flex items-center gap-1.5 font-semibold text-ink"><x-icon name="chart-bar" class="h-4 w-4 text-tone-green" />Upload data hasil iklan (.xls/.xlsx)</span><div class="flex items-center gap-2"><input type="file" name="ad_leads_file" accept=".xls,.xlsx" class="rounded-lg border-border bg-surface-muted"><a href="{{ route('anggaran.leads.template') }}" class="whitespace-nowrap rounded-lg border border-border px-3 py-2 text-sm">Download Template</a></div><span class="text-xs text-ink-muted">Kosongkan jika tidak ingin menambah data hasil iklan.</span></label>
                    </div>
                @endif
