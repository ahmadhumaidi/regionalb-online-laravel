<x-layouts.app title="BDC Marketing" active="bdc-users">
    ...
    <section class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">@foreach([['Staff Terbaca',$totals['staff']],['Total Data',$totals['total']],['Closing',$totals['closing']],['FU Hari Ini',$totals['fu_hari_ini']]] as [$label,$value])<div class="rounded-2xl glass-card p-4"><p class="text-xs text-ink-muted">{{ $label }}</p><p class="mt-1 text-lg font-semibold text-ink">{{ number_format($value,0,',','.') }}</p></div>@endforeach</section>
