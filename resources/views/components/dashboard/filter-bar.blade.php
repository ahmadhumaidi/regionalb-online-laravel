...
<form method="GET" action="{{ route('dashboard') }}" class="mb-6 flex flex-wrap items-end gap-3 rounded-2xl border border-border bg-surface p-4">
    <div>
    ...
    @if ($user->role === 'staff')
        ...
        <div>
            ...
            <input value="{{ $user->campus_name }}" readonly title="Otomatis sesuai akun login" class="rounded-lg border border-border bg-surface-muted px-3 py-2 text-sm text-ink-muted">
        </div>
        <div>
